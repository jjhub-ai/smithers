...
components:
  ReviewCycle:
    ...
    steps:
      ...
      - id: "{id}-revise"
        prompt: |
          Revise based on feedback: {params.feedback}
        output:
          content: string
...
components:
  Summarizer:
    ...
    steps:
      - id: "{id}-summarize"
        prompt: |
          Summarize the following text in at most {params.maxWords} words.
  ...
  GuardedDeploy:
    ...
    steps:
      ...
      - id: "{id}-deploy"
        needs: ["{id}-approve"]
        run: |
          await deployToEnv(params.env, params.version);
        output:
          url: string
```
