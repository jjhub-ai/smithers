...
A `.toon` file describes a workflow as structured YAML-like blocks with embedded prompts and optional inline TypeScript. Smithers compiles it into a typed workflow graph at build time.
...
steps:
  - id: research
    agent: researcher
    prompt: |
      Research the following topic and provide a summary with key points.
      Topic: {input.topic}
    output:
      summary: string
      keyPoints: string[]
  ...
  - id: report
    agent: writer
    prompt: |
      Write a report based on this research.
  ...
  - kind: loop
    ...
    children:
      - id: review
        agent: reviewer
        prompt: |
          Review this draft: {draft.content}
        output:
          approved: boolean
          feedback: string
...
steps:
  ...
  - id: deploy
    ...
    run: |
      return { url: "https://app.example.com", deployedAt: new Date().toISOString() };
    output:
      url: string
      deployedAt: string
```
