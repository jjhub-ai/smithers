...
steps:
  - id: analyze
    prompt: |
      Analyze this bug: {input.description}
    output:
      summary: string
...
steps:
  - id: plan
    prompt: |
      Create a fix plan for: {input.description}
    output:
      plan:
        steps: string[]
...
imports:
  schemas:
    - from: "./schemas.ts"
      use: [TicketInput, AnalysisOutput]
