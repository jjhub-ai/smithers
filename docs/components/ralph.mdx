...
- **Direct nesting throws.** Placing a `<Ralph>` as an immediate child of another `<Ralph>` throws `"Nested <Ralph> is not supported."`. Wrap the inner loop in a `<Sequence>` to nest loops (see [Nested loops](#nested-loops) above).
