...
Smithers calls your builder function with the current `SmithersCtx`, which contains the run's input and all outputs produced so far. The JSX you return is rendered through a [custom React reconciler](/docs/concepts/renderer-internals) -- not React DOM -- producing an in-memory `HostElement` tree.
